...
void setup() {
  ...
  if (strlen(FIREBASE_AUTH) == 0 || strcmp(FIREBASE_AUTH, "YOUR_DATABASE_SECRET_HERE") == 0) {
...
void startAPMode() {
  ...
  macStr.toUpperCase();
  String apSSID = AP_SSID_PREFIX + macStr.substring(0, 4);
...
void handleRoot() {
  ...
  html += "<title>PillMate WiFi Setup</title>";
  html += "<style>";
  ...
  html += "select, input { width: 100%; padding: 12px; margin: 5px 0; border: 2px solid #ddd; border-radius: 5px; box-sizing: border-box; font-size: 16px; }";
  html += "select:focus, input:focus { outline: none; border-color: #007AFF; }";
  html += "button { width: 100%; padding: 14px; background: #007AFF; color: white; border: none; border-radius: 5px; font-size: 16px; font-weight: bold; cursor: pointer; margin-top: 10px; }";
  html += "button:hover { background: #0056b3; }";
  html += "button:disabled { background: #ccc; cursor: not-allowed; }";
  html += ".refresh-btn { background: #28a745; margin-top: 5px; }";
  html += ".refresh-btn:hover { background: #218838; }";
  html += ".status { margin-top: 20px; padding: 12px; border-radius: 5px; }";
  html += ".success { background: #d4edda; color: #155724; border: 1px solid #c3e6cb; }";
  html += ".error { background: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; }";
  html += ".info { background: #d1ecf1; color: #0c5460; border: 1px solid #bee5eb; }";
  html += ".signal { display: inline-block; margin-left: 10px; font-size: 12px; }";
  html += ".signal.strong { color: #28a745; }";
  html += ".signal.medium { color: #ffc107; }";
  html += ".signal.weak { color: #dc3545; }";
  html += "</style></head><body>";
  html += "<div class='container'>";
  html += "<h1>🔌 PillMate WiFi Setup</h1>";
  ...
  html += "</select>";
  html += "<button type='button' class='refresh-btn' onclick='scanNetworks()'>🔄 Refresh List</button>";
  html += "</div>";
  html += "<div class='form-group'>";
  html += "<label>WiFi Password:</label>";
  html += "<input type='password' id='password' name='password' placeholder='Enter password (if required)'>";
  html += "</div>";
  html += "<button type='submit' id='connectBtn'>Connect</button>";
  html += "</form>";
  ...
  html += "  if(rssi > -50) return '<span class=\"signal strong\">● Strong</span>';";
  html += "  if(rssi > -70) return '<span class=\"signal medium\">● Medium</span>';";
  html += "  return '<span class=\"signal weak\">● Weak</span>';";
  html += "}";
  html += "function populateNetworks() {";
  html += "  const select = document.getElementById('ssidSelect');";
  ...
  html += "      status.innerHTML = '✅ Found ' + networks.length + ' networks';";
  html += "      document.getElementById('connectBtn').disabled = false;";
  html += "    })";
  html += "    .catch(err => {";
  html += "      status.className = 'status error';";
  html += "      status.innerHTML = '❌ Error scanning: ' + err;";
  html += "      document.getElementById('connectBtn').disabled = false;";
  html += "    });";
  html += "}";
  ...
  html += "  const ssid = document.getElementById('ssidSelect').value;";
  html += "  const password = document.getElementById('password').value;";
  html += "  if(!ssid) {";
  html += "    status.className = 'status error';";
  html += "    status.innerHTML = '❌ Please select a network';";
  html += "    return;";
  html += "  }";
  html += "  status.className = 'status info';";
  ...
  html += "        status.className = 'status success';";
  html += "        status.innerHTML = '✅ Connected! Device is connecting to WiFi...<br>You can close this page.';";
  html += "      } else {";
  html += "        status.className = 'status error';";
  html += "        status.innerHTML = '❌ Error: ' + data.message;";
...
void handleSave() {
  if (server.hasArg("ssid")) {
    ...
    if (ssid.length() == 0) {
      String json = "{\"success\":false,\"message\":\"Please select a network\"}";
    ...
    String json = "{\"success\":true,\"message\":\"WiFi credentials saved\"}";
    server.send(200, "application/json", json);
