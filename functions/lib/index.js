...
exports.getSchedule = functions.https.onRequest(async (req, res) => {
    try {
        ...
        if (dev.apiKey !== apiKey) {
            res.status(403).json({ ok: false, error: "Bad apiKey" });
            return;
        }
        if (!dev.ownerUid) {
            res.status(400).json({
        ...
        const schedSnap = await db
            .collection(`users/${dev.ownerUid}/schedule`)
            .orderBy("time")
            .get();
        const schedule = schedSnap.docs.map((d) => (Object.assign({ id: d.id }, d.data())));
        res.json({
            ok: true,
            deviceId,
            ownerUid: dev.ownerUid,
            schedule,
        });
    }
    catch (e) {
